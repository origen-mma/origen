...
To quantify the temporal discrimination power of the SVI $t_0$ fitter, we simulate 50 kilonova and 50 supernova light curves with realistic ZTF cadence (1--3 day revisit) and SNR~$=20$, fit each with the MetzgerKN SVI model, and compare the recovered $t_0$ against the true value. Figure~\ref{fig:t0} summarizes the results.

\begin{figure*}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/t0_constraint_validation.pdf}
    \caption{Explosion time ($t_0$) recovery validation. \emph{Top left}: Distribution of $|t_0^\mathrm{true} - t_0^\mathrm{fit}|$ for kilonovae (red) and supernovae (orange), with median values indicated by dashed lines. \emph{Top right}: Cumulative distribution function; the green-shaded region marks the $\pm 1$~day GW correlation window. \emph{Bottom left}: Fitted $t_0$ uncertainty vs.\ actual error, showing that uncertainties are well-calibrated for kilonovae after $4\times$ sigma inflation (median uncertainty 0.76~days vs.\ median error 0.65~days). \emph{Bottom right}: Simulated temporal discrimination showing the distribution of fitted $t_0$ values relative to the GW merger time; kilonovae cluster near $t_0 = 0$ while supernovae are uniformly distributed. \label{fig:t0}}
\end{figure*}

Kilonovae are recovered with a median $t_0$ error of 0.65~days; 72\% of fitted $t_0$ values fall within 1~day of the true explosion time. The fitted $t_0$ uncertainty (median 0.76~days) is well-calibrated against the actual error thanks to the $4\times$ sigma inflation, validating the posterior calibration. Supernovae are recovered with a median $t_0$ error of 0.56~days for the well-constrained majority, though a long tail of poorly-constrained fits (RMS 10.3~days) reflects the intrinsic difficulty of constraining explosion time from the declining portion of a slowly-evolving light curve. To quantify temporal discrimination power, we simulate 10,000 kilonova and supernova events (Figure~\ref{fig:t0}, bottom right): kilonova $t_0$ values are drawn from $\mathcal{N}(0, 0.65\,\mathrm{d})$ representing prompt merger counterparts, while supernova true explosion times are uniformly distributed over a 30-day baseline with fitted $t_0$ errors drawn from $\mathcal{N}(0, 31\,\mathrm{d})$. Of the simulated kilonovae, 44\% have fitted $t_0$ within the $[0, 1]$~day coincidence window, compared to only 1.1\% of supernovae, yielding a temporal discrimination factor of ${\sim}40\times$.

Combined with the spatial discrimination (mean signal-to-background ratio ${\sim}10\times$ for optical), the total spatio-temporal rejection is ${\sim}420\times$ ($10\times$ spatial $\times$ $42\times$ temporal) when SVI fitting succeeds. Even accounting for the ${\sim}10{,}000\times$ higher supernova rate relative to kilonovae, the combined spatial plus temporal cuts reduce the expected number of false coincidences to ${\sim}2$ per GW event at ZTF rates, manageable for human-in-the-loop vetting. When SVI fitting fails, the per-measurement fallback gives ${\sim}80\times$ total rejection (${\sim}13$ false associations), with light curve classification (Section~\ref{sec:earlyrate_results}) providing modest additional discrimination.
...
\begin{table*}[htbp]
\centering
\caption{End-to-end background rejection for optical counterpart identification, starting from ${\sim}1{,}000$ ZTF candidates per GW error region. The ``With SVI'' column assumes reliable $t_0$ recovery; the ``Per-measurement fallback'' column reflects the case when SVI fails and temporal matching uses the raw detection window. \label{tab:combined}}
\begin{tabular}{lcccc}
\toprule
...
Temporal ($t_0$ recovery) & 44.6\% KN vs.\ 1.1\% SN in $[0, 1]$~d & $42\times$ & 2.4 & --- \\
Temporal (per-measurement) & Detection within $\pm 1$~d window & $7.7\times$ & --- & 12.8 \\
Early rate (soft scoring) & $\langle f_\mathrm{SN}\rangle / \langle f_\mathrm{KN}\rangle = 1.15$ & $1.2\times$ & 2.1 & 11.1 \\
\midrule
\textbf{Cumulative} & & & \textbf{${\sim}480\times$} & \textbf{${\sim}80\times$} \\
\bottomrule
\end{tabular}
\end{table*}
...
\textbf{Temporal rejection.} When SVI $t_0$ fitting succeeds (median KN recovery error 0.65~days, with calibrated uncertainty of 0.76~days), the temporal coincidence window $[0, 1]$~day admits 44.6\% of kilonovae but only 1.1\% of supernovae (whose true explosion times are distributed over a 30-day baseline), yielding a $42\times$ rejection factor and ${\sim}2.4$ expected false associations. However, SVI fitting requires well-sampled light curves; when it fails (uncertainty $\sigma_{t_0} > 1$~day), the correlator falls back to per-measurement matching, which accepts any candidate with a detection within 1~day of the GW trigger. Background supernovae have a ${\sim}2/30 \approx 7\%$ chance of a detection in this window given typical ZTF cadence, giving a more modest $7.7\times$ rejection and ${\sim}12.8$ expected false associations.
...
\textbf{End-to-end.} The cumulative spatio-temporal rejection of ${\sim}480\times$ (with SVI) reduces 1,000 ZTF candidates to ${\sim}2$ expected false associations per GW event, a manageable number for human-in-the-loop vetting via platforms such as SkyPortal \citep{Coughlin2023}. In the per-measurement fallback mode (${\sim}80\times$ rejection, ${\sim}13$ false associations), additional vetting is required but the candidate list remains tractable. At LSST rates (${\sim}10{,}000$ candidates), the SVI pathway yields ${\sim}20$ false associations, still within reach of automated follow-up prioritization.
