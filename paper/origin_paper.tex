...
The binary neutron star merger GW170817 \citep{Abbott2017a, Abbott2017b} and its electromagnetic counterparts---the short gamma-ray burst GRB~170817A \citep{Goldstein2017, Abbott2017c}, the kilonova AT~2017gfo \citep{Coulter2017, Kasen2017, Villar2017}, and a multi-wavelength afterglow---opened the era of multi-messenger astronomy with gravitational waves, yielding constraints on the neutron-star equation of state \citep{Dietrich2020}, an independent Hubble constant measurement, and direct evidence for $r$-process nucleosynthesis \citep{Lattimer1974, Siegel2019, Levan2024}.

As the LIGO--Virgo--KAGRA (LVK) detector network advances from its fourth (O4) to fifth (O5) observing run, the rate of detectable compact binary coalescences continues to grow \citep{Abbott2020, GWTC4, GWTC4pop}, with several BNS mergers expected per year at O5 sensitivity. The LVK low-latency infrastructure now issues public alerts within seconds of detection \citep{Chaudhary2024, LVK2019lowlatency}. Simultaneously, wide-field optical surveys---the Zwicky Transient Facility \citep[ZTF;][]{Bellm2019, Graham2019}, ATLAS \citep{Tonry2018}, and GOTO \citep{Steeghs2022}---detect thousands of transients per night, and the Vera C.\ Rubin Observatory LSST \citep{Ivezic2019} will increase this volume by an order of magnitude. In the gamma-ray domain, \textit{SVOM} \citep{Wei2016} provides ${\sim}60$ GRBs per year, complementing \textit{Fermi} and \textit{Swift}, while \textit{Einstein Probe} \citep{EinsteinProbe2025} adds X-ray transients at roughly one per ten days. Together, these facilities create a rich but fragmented multi-messenger landscape in which rare astrophysical associations must be identified against a large combinatorial background.
...
Despite these advances, no existing public system simultaneously ingests alerts from multiple Kafka brokers with heterogeneous authentication, performs HEALPix-based spatial correlation using full GW probability skymaps, applies light-curve-based source classification to discriminate kilonovae from supernovae, and computes a unified joint FAR statistic. Multi-messenger follow-up remains structured around isolated alerts from individual discovery streams, lacking a framework that associates heterogeneous signals with a single, evolving astrophysical origin in real time.

In this paper, we present ORIGIN, a Rust-based multi-messenger superevent manager that fills this gap. ORIGIN adopts a \textit{superevent} paradigm in which alerts from gravitational-wave detectors, gamma-ray satellites, X-ray telescopes, and optical surveys are continuously associated with a common astrophysical origin and updated as new observations arrive. Section~\ref{sec:architecture} describes the system architecture. Section~\ref{sec:correlation} details the correlation algorithms. Section~\ref{sec:lightcurve} presents the light curve analysis pipeline. Section~\ref{sec:calibration} reports the FAR calibration on an O4 observing scenario population. Section~\ref{sec:discussion} discusses performance, comparison with existing systems, and future directions.
...
The \texttt{mm-core} crate defines shared types and algorithms; \texttt{mm-correlator} builds on these to implement the temporal indexing and superevent aggregation of Section~\ref{sec:correlation}. Two ingestion crates, \texttt{mm-gcn} and \texttt{mm-boom}, handle protocol-specific consumption from NASA's GCN and Caltech's BOOM Kafka brokers, respectively. \texttt{mm-simulation} provides synthetic event generation, while \texttt{mm-config}, \texttt{mm-redis}, and \texttt{mm-api} supply configuration, persistence, and external access. \texttt{mm-service} packages the system into deployable CLI binaries.

\subsection{Data Ingestion} \label{sec:ingestion}

ORIGIN consumes two real-time Kafka streams. The first is the General Coordinates Network (\texttt{kafka.gcn.nasa.gov}), from which ORIGIN subscribes to LVK gravitational-wave alerts \citep{Chaudhary2024, LVK2019lowlatency} on \texttt{igwn.gwalert} (JSON with skymap URLs), Fermi-GBM GRB positions on \texttt{gcn.classic.voevent.FERMI\_GBM\_*} (VOEvent XML), Swift-BAT GRBs on \texttt{gcn.notices.swift.bat.guano} (Avro), \textit{Einstein Probe} \citep{EinsteinProbe2025} X-ray transients on \texttt{gcn.notices.einstein\_probe.wxt.alert}, and IceCube neutrino tracks on \texttt{gcn.notices.icecube.*}. Authentication uses OAUTHBEARER with a custom token-refresh callback, bypassing librdkafka's built-in OIDC implementation.
...
From the GP posterior evaluated at 50 uniformly spaced points, the system extracts: rise and decay rates (linear regression slopes over the first and last 25\% of predictions), peak magnitude, FWHM (duration at half peak flux above baseline), and flux derivatives $df/dt$ at all prediction points. These features are combined into a background rejection score that multiplies the joint FAR.
...
The second approach uses Stochastic Variational Inference (SVI) to fit a parametric light curve model. For the MetzgerKN model \citep{Metzger2010}, the parameters are ejecta mass $M_\mathrm{ej}$, velocity $v_\mathrm{ej}$, opacity $\kappa$, and explosion time $t_0$. The procedure has three stages:

\emph{Stage 1: Initialization.} Particle Swarm Optimization (PSO) with 40 particles explores the parameter space for 100--200 iterations. For the Bazin model, a preliminary grid search over $(t_0, \tau_\mathrm{rise}, \tau_\mathrm{fall})$ with analytically-solved amplitude constrains the PSO bounds and breaks strong $t_0$--timescale degeneracies.

\emph{Stage 2: SVI refinement.} A mean-field Gaussian variational distribution $q(\theta) = \prod_i \mathcal{N}(\mu_i, \sigma_i^2)$ is optimized via the reparameterization trick \citep{Kingma2014} using Adam \citep{Kingma2015} for 1,000--5,000 iterations, maximizing the ELBO. The variational standard deviations are inflated by $4\times$ post-optimization to correct for mean-field underestimation of posterior variance (calibrated via P-P plots on synthetic light curves, following the ZTF lightcurve-fitting pipeline).

\emph{Stage 3: Profile $t_0$ refinement.} For empirical models (Bazin, Villar, PowerLaw), $t_0$ is swept on a 50-point grid while holding other parameters at their SVI means, analytically re-fitting the amplitude via weighted least squares at each grid point. The profile peak gives a bias-corrected $t_0$; parabolic interpolation of the curvature gives a calibrated $1\sigma$ uncertainty. This costs ${\sim}50$ model evaluations, negligible compared to ${\sim}80{,}000$ in Stage~2.
...
For Fermi-GBM, we adopt an error radius of $13.2\arcdeg$, derived from 5,832 real Fermi-GBM detections in the VOEvent archive---$2.6\times$ larger than the commonly cited ${\sim}5\arcdeg$, which significantly affects spatial FAR calculations. Despite this large error circle, the median signal probability ($P_\mathrm{spatial} = 0.118$) exceeds the background median ($1.3\times10^{-6}$) by ${\sim}9\times10^4$. All 178 signal trials yield nonzero spatial probability, while 32.4\% of background trials return zero. The mean signal-to-background ratio is $12.8\times$, lower than the median ratio because the latter is dominated by background trials with negligible probability; both metrics are relevant for typical versus worst-case discrimination.

Swift-BAT achieves even stronger discrimination. Its 2~arcmin error circle yields lower absolute spatial probabilities (median signal $P_\mathrm{spatial} = 2.66\times10^{-5}$), but the background median drops to zero, with 68.0\% of background trials having no skymap overlap. The mean signal-to-background ratio ($10.1\times$) is comparable to Fermi-GBM, while the formally infinite median ratio reflects that most random sky positions have zero probability at this angular resolution. Both Swift-BAT's 2~arcmin error circle and the 2~arcsecond optical position error fall below the HEALPix pixel scale at $\mathrm{NSIDE}=512$ (${\sim}6.9$~arcmin), so both reduce to single-pixel queries with very similar spatial probability distributions.
...
The optical channel yields a spatial probability distribution nearly identical to Swift-BAT, as expected: both have error regions smaller than the HEALPix pixel size, so both reduce to single-pixel queries. All kilonova signal trials have nonzero spatial probability, while 68.1\% of supernova background trials have zero overlap with the GW skymap. At higher skymap resolution ($\mathrm{NSIDE} = 2048$ or 4096, as sometimes used in LVK skymaps), the tighter optical localization would provide stronger discrimination than Swift-BAT.
...
Kilonovae are recovered with a median $t_0$ error of 0.61~days, with 70\% falling within 1~day of the true explosion time. The fitted uncertainty (median 0.73~days) is well-calibrated against the actual error after $4\times$ sigma inflation, validating the posterior calibration. Supernovae have a median error of 0.58~days for the well-constrained majority, though a long tail (RMS 9.8~days) reflects the difficulty of constraining $t_0$ from the declining portion of a slowly-evolving light curve.

To quantify temporal discrimination, we simulate 10,000 events per class (Figure~\ref{fig:t0}, bottom right): kilonova $t_0$ values are drawn from $\mathcal{N}(0, \sigma_\mathrm{KN})$ with $\sigma_\mathrm{KN} = 0.61\,\mathrm{d}$ (the median recovery error), while supernova true explosion times are drawn uniformly over a 30-day baseline and perturbed by $\mathcal{N}(0, \sigma_\mathrm{SN})$. The resulting temporal discrimination factor and population fractions within the $[0, 1]$~day coincidence window are reported in Figure~\ref{fig:combined}.
...
The classifier achieves high specificity: zero supernovae of either type are falsely boosted ($f < 1$), while 99.8\% of SN~Ia and 97.4\% of SN~II are correctly penalized ($f > 1$). However, sensitivity for kilonovae is limited at ZTF cadence: only 5.8\% receive a boost, while 68\% are penalized and 26\% remain neutral. Kilonovae peak within $<1$~day, often before the second observation, leaving the rise rate poorly constrained. The decay rate (median 0.39~mag/day for KN vs.\ 0.07~mag/day for SN~Ia) provides better discrimination in principle, but the 3-day window often captures too few post-peak measurements.

Consequently, the mean KN FAR multiplier (5.0) is comparable to the SN~Ia multiplier (5.7), yielding only ${\sim}1.1\times$ net discrimination. The classifier's primary value is as a \emph{positive identifier}: any source receiving $f < 1$ is almost certainly a fast transient, even though most kilonovae will not receive this boost at survey cadence. A hard-cut mode rejects 99.8\% of SN~Ia but removes 68\% of kilonovae---too aggressive for survey cadence but useful for triggered follow-up with sub-day cadence.
...
For Fermi-GBM, the RAVEN FAR yields a median of $2.4\times10^{-3}$~yr$^{-1}$ for signal trials, with 98.9\% below the 1/yr significance threshold---confirming that despite the $13.2\arcdeg$ error circle, Fermi-GBM correlations are significant event-by-event. For Swift-BAT, the 2~arcmin error circle yields lower $P_\mathrm{spatial}$ and consequently higher RAVEN FAR values (median 11.4~yr$^{-1}$). This apparent paradox reflects Equation~\ref{eq:farraven}: FAR~$\propto P_\mathrm{spatial}^{-1}$, so instruments with small error circles yield lower $P_\mathrm{spatial}$ and higher FAR despite excellent spatial discrimination. In practice, Swift-BAT's tight localization provides complementary information through the empirical discrimination ratio.

\subsection{Combined Background Rejection} \label{sec:combined}

We compute end-to-end background rejection by propagating ${\sim}1{,}000$ expected ZTF transients within a typical 100~deg$^2$ GW error region through successive pipeline stages, tracking expected false associations rather than multiplying independent factors (which risks double-counting). Table~\ref{tab:combined} and Figure~\ref{fig:combined} show results for two modes: SVI $t_0$ fitting (reliable uncertainty $<1$~day) and a per-measurement temporal fallback.
...
Table~\ref{tab:comparison} compares ORIGIN with RAVEN \citep{Urban2016} and Treasure Map \citep{Wyatt2020}. RAVEN provides real-time GW+GRB coincidence searches and a joint FAR statistic within the LVK infrastructure \citep{Chaudhary2024}, but does not ingest optical streams or classify light curves. Treasure Map coordinates EM follow-up but operates as a coordination tool rather than a correlation engine. Alert brokers such as AMPEL \citep{Nordin2019} and Fink \citep{Moller2021} classify optical transients but lack joint GW+EM significance calculations; SkyPortal \citep{Coughlin2023} aggregates multi-messenger data but lacks automated FAR computation. ORIGIN combines RAVEN's joint FAR formalism with multi-broker ingestion and automated source classification, and is fully open-source. Its RAVEN-compatible FAR enables direct comparison of significance estimates. The recent candidate superkilonova associated with the subthreshold trigger S250818k \citep{Kasliwal2025} underscores the urgency of systems that can rapidly correlate sub-threshold GW events with optical transients.
...
The gap between the SVI and per-measurement fallback pathways (${\sim}200\times$ vs.\ ${\sim}79\times$ rejection) highlights the importance of $t_0$ recovery. SVI fitting requires $\gtrsim 5$ detections spanning the rise and decline, and many early-time light curves at ZTF cadence will fall to the weaker per-measurement mode. Quantifying the SVI success fraction as a function of observing strategy is an important target for future work.

The early rate discriminator has high specificity (zero false boosts) but low sensitivity at ZTF cadence: only 5.8\% of kilonovae receive a positive boost, because the peak typically occurs between survey visits. At sub-day cadence---as expected for triggered follow-up---the sensitivity should improve substantially, but this has not yet been validated.

Computationally, the MetzgerKN SVI fitter requires ${\sim}24$~s per light curve; a fast-mode configuration (1,000 vs.\ 5,000 SVI iterations) reduces this to ${\sim}5$~s with moderate accuracy loss. The Bazin fitter with profile $t_0$ refinement averages ${\sim}5$~s, benefiting from the cheaper analytical amplitude refit.

The spatial analysis was performed at $\mathrm{NSIDE}=512$, where both Swift-BAT (2~arcmin) and optical (2~arcsec) error regions fall within a single pixel; higher-resolution skymaps would improve optical discrimination. The $t_0$ validation used 50 light curves per class, giving ${\sim}14\%$ relative statistical uncertainty on recovery fractions. The O4 calibration uses simulated counterparts rather than live GCN data, since the MDC events lack full HEALPix skymaps---though skymap retrieval from GraceDB is fully supported.

Looking ahead, we plan several extensions for the Rubin--\textit{SVOM}--O5 era. Integration with the NMMA Bayesian framework \citep{Pang2023} will replace empirical rate classifiers with full posterior inference on ejecta properties and progenitor class. We will add LSST alert streams via Fink \citep{Moller2021} and implement GPU-accelerated SVI for real-time classification at LSST rates. Deeper SkyPortal integration \citep{Coughlin2023} will enable human-in-the-loop vetting and dynamic observing prioritization, while Apache Beam event-time semantics will support adaptive coincidence windows. We also plan to extend the correlation framework to neutrino + GW associations via IceCube real-time tracks and to natively support \textit{SVOM} \citep{Wei2016} and \textit{Einstein Probe} \citep{EinsteinProbe2025} streams.

\section{Conclusion} \label{sec:conclusion}

We have presented ORIGIN, a real-time multi-messenger superevent manager that correlates gravitational-wave alerts with electromagnetic and neutrino counterparts through a unified framework, ingesting alerts from GCN and BOOM/ZTF Kafka streams with heterogeneous authentication.

Calibration on 178 O4-scenario BNS/NSBH events demonstrates strong performance. Spatial discrimination achieves ${\sim}10\times$ mean signal-to-background ratio for optical counterparts and ${\sim}9\times10^4$ median ratio for Fermi-GBM, with 68\% of background positions having zero skymap probability at optical angular scales. SVI-based $t_0$ recovery---with $4\times$ sigma inflation and profile-likelihood refinement---provides ${\sim}17\times$ temporal rejection (45\% of kilonovae vs.\ 2.6\% of supernovae within the $[0, 1]$~day window), while early rate classification penalizes $>97\%$ of supernovae with zero false boosting. End-to-end, the pipeline achieves ${\sim}200\times$ cumulative rejection when SVI succeeds (${\sim}5$ false associations per GW event from 1,000 ZTF candidates), or ${\sim}79\times$ in per-measurement fallback (${\sim}13$ false associations).
