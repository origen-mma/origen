...
fn main() -> Result<()> {
    ...
    info!("  Time window: {:.0} days after GW trigger", time_window_days);
    ...
    info!("  Survey coverage: {:.1}% of sky", ztf_config.survey_coverage * 100.0);
    info!("  Limiting magnitude: {:.1}", ztf_config.limiting_magnitude);
    info!("  Shock cooling fraction: {:.1}%\n", ztf_config.shock_cooling_fraction * 100.0);

    let ztf_transients = generate_background_optical(&ztf_config, o4_start_gps, o4_end_gps, &mut rng);
    ...
    info!("  Shock cooling: {} ({:.2}%)", shock_cooling_count,
    ...
    info!("Creating {} mock BNS events uniformly distributed in O4...", expected_bns_events);
    ...
    info!("Total background transients: {}", ztf_stats.total_background_transients);
    info!("Temporal coincidences (within {} days): {}",
          time_window_days, ztf_stats.temporal_coincidences);
    info!("Spatio-temporal coincidences: {}", ztf_stats.spatial_temporal_coincidences);
    info!("  └─ Shock cooling: {}", ztf_stats.shock_cooling_coincidences);
    info!("  └─ SNe Ia: {}", ztf_stats.sne_ia_coincidences);
    info!("Expected false associations (analytical): {:.3}",
          ztf_stats.expected_false_associations);
    info!("Chance rate per GW: {:.3}%", ztf_stats.chance_rate_per_gw * 100.0);
    ...
    info!("\nTotal rejection efficiency: {:.4}%", total_rejection * 100.0);
    ...
    info!("  • Temporal cut (14 days): {:.2}% rejected", temporal_rejection * 100.0);
    info!("  • Spatial cut (100 sq deg): {:.2}% of temporal coincidences rejected",
          spatial_temporal_rejection * 100.0);
    info!("  • Overall: {:.4}% of ALL transients rejected", total_rejection * 100.0);

    info!("\nConclusion:");
    info!("  Time + spatial cuts are EXTREMELY effective!");
    info!("  False optical associations are rare (~{:.1}% per BNS)",
          ztf_stats.chance_rate_per_gw * 100.0);
    info!("  Shock cooling transients: {} false coincidences",
          ztf_stats.shock_cooling_coincidences);
    info!("  SNe Ia transients: {} false coincidences", ztf_stats.sne_ia_coincidences);
