...
fn main() -> Result<()> {
    ...
    let config = SimulationConfig {
        injections_file: PathBuf::from("/Users/mcoughlin/Code/ORIGIN/observing-scenarios/runs/O4HL/bgp/injections.dat"),
        skymap_dir: PathBuf::from("/Users/mcoughlin/Code/ORIGIN/observing-scenarios/runs/O4HL/bgp/allsky"),
        grb_xml_dir: PathBuf::from("/Users/mcoughlin/Code/ORIGIN/growth-too-marshal-gcn-notices/notices"),
    ...
    info!("  Time offset range: ±{:.0} seconds", config.time_offset_range);
    ...
    for (i, result) in results.iter().take(10).enumerate() {
        info!("Simulation {}:", i);
        info!("  GW Position: (RA={:.2}°, Dec={:.2}°)", result.gw_ra, result.gw_dec);
        info!("  GRB Position: (RA={:.2}°, Dec={:.2}°)", result.grb_ra, result.grb_dec);
        ...
        info!("  Correlated: {}", if result.correlated { "✅" } else { "❌" });
        info!("  Spatial significance: {:.6e}\n", result.spatial_significance);
    }
