...
```
P(d) ∝ d²    →    CDF: F(d) = (d/d_max)³    →    d = d_max × u^(1/3)
```
...
```
cos(i) ~ Uniform(-1, 1)    →    i = arccos(cos_i)
```
...
A `MockSkymap` is generated for each event with area scaling as distance²:

```
radius_90 = 3° × (d / 40 Mpc)    capped at 30°
```
...
BNS ejecta properties are computed using the Krüger & Foucart (2020) fitting formulae:
...
2. **Apparent magnitude**: M_peak + distance_modulus + model shape correction
3. **Detection threshold**: SNR ≥ 3 required (SNR = 5 × 10^((m_lim - m)/2.5))
4. **Photometric noise**: Gaussian in magnitude space, combining systematic floor and photon noise:
   ```
   σ_mag = √(σ_floor² + (1.0857/SNR)²)
   ```
